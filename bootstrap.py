...
def _config_path():
    """Path to the workshop config file saved alongside bootstrap.py."""
    return os.path.join(os.path.dirname(os.path.abspath(__file__)),
                        "workshop-config.json")


def save_workshop_config(args, selected_tz=None):
    """Persist bootstrap connection args so other scripts can reuse them."""
    cfg = {
        "host":          args.host,
        "user":          args.user,
...
def load_workshop_config():
    """Load previously saved bootstrap config.  Returns dict or None."""
    p = _config_path()
